...
```git commit -e [--file=<file>| -m <msg> ]``` - performs a commit while allowing you to edit the messages taken from the various sources e.g. file ```<file>```, commit message ```<msg>```.
